    ...
    $.ajax({
        ...
        $.each(data.days , function (key2, value2) {
            ...
            <div class="row">
                <div class="col-md-12 mt-3 ">
                    <select class="subject"  name="subject_id[`+value.id+`][`+value2.id+`]" data-start="`+value.start_time+`"  data-end="`+value.end_time+`"   data-day="`+value2.id+`"  id ="subject"  data-size="5" data-style="btn btn-secondary" title="Select Subject">
        ...
        */
        ...
        $.each(data.subjects , function (keysubject, valuesubject) {       
           ...
           if (data.list.length != 0) {
                    console.log(data.list[value.id][value2.id]['emp_Id']);
                    var teachervee =data.list[value.id][value2.id]['emp_Id']?data.list[value.id][value2.id]['emp_Id']:'';

                    var teacherveename =data.list[value.id][value2.id]['teacher']?data.list[value.id][value2.id]['teacher']:'';
...
$(document).on("change",".subject", function(event)
{
    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if(result.visibility || flag ){

                if(result.visibility){
                alert('Teacher is busy on this time slote')
                }
