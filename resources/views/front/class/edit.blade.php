                    ...
                    <div class="card-body">
                        ...
                        @endif
                         <form action="{{url('class/update')}}" method="Post">
                             @csrf
                             <input type="hidden" name="id" value="{{$class->cls_Id}}">
                                <div class="">
                                    <div class="row">
                                        <div class="col-sm-12">
                                             <div class="row">
                                                <h6 class="col-sm-12 text-center">Edit Class Details</h6>
                                                <div class="form-group col-sm-6">
                                                    <label for="add-class-name">Class Name</label>
                                                    <input type="text" class="form-control" id="add-class-name" placeholder="" name="class_name" value="{{$class->class}}">
                                                </div>
                                                <div class="form-group col-sm-6">
                                                    <label for="add-numeric-name">Numeric Name</label>
                                                    <input type="text" id="add-numeric-name" class="form-control" placeholder="" name="numeric_name" value="{{$class->numeric_name}}">
                                                </div>
                                                <div class="form-group col-sm-6">
                                                    <label for="add-no-of-period">No of Periods</label>
                                                    <input type="text" id="add-no-of-period" class="form-control" placeholder="" name="no_of_period" value="{{$class->no_of_period}}">
                                                </div>
                                                <div class=" col-sm-6 select-wizard">
                                                    ...
                                                    <select class="form-group js-example-basic-single"  id="add-teacher" name="teacher"  data-style="btn btn-secondary" title="Select Billing Scgedule" >
                                                        ...
                                                        <option value="1" @if($class->teacher==1)  selected @endif>Ahmed</option>
                                                        <option value="2" @if($class->teacher==2)  selected @endif>Jaffar</option>
                                                        <option value="3" @if($class->teacher==3)  selected @endif>Muneer</option>
                                                        <option value="4" @if($class->teacher==4)  selected @endif>Saleem</option>
                                                        <option value="5" @if($class->teacher==5)  selected @endif>Awais</option>
                                                        <option value="6" @if($class->teacher==6)  selected @endif>Ahmed</option>
                                                        <option value="7" @if($class->teacher==7)  selected @endif>Jaffar</option>
                                                        <option value="8" @if($class->teacher==8)  selected @endif>Muneer</option>
                                                        <option value="9" @if($class->teacher==9)  selected @endif>Saleem</option>
                                                        <option value="10" @if($class->teacher==10)  selected @endif>Awais</option>
                                                    </select>
                                                </div>
                                                <div class=" col-sm-6 select-wizard">
                                                    ...
                                                    <select class="form-control js-example-basic-single" id="school-section" name="school_section"  data-style="btn btn-secondary" title="Select Teacher" >
                                                        ...
                                                        <option value="{{$school_section->sc_sec_Id}}" @if($class->sc_sec_Id==$school_section->sc_sec_Id) selected @endif>{{$school_section->sc_sec_name}}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div class="form-group col-sm-6">
                                                    <label for="add-tuition-fee">Tuition Fee</label>
                                                    <input type="text" id="add-tuition-fee" class="form-control" placeholder="" name="tuition_fee" value="{{$class->tuition_fee}}">
                                                </div>
                                                 @php
                                                    $subs = explode(" ",$class->subject);
                                                    //dump($subs);
                                                    foreach($subs as $sublist){ echo $sublist; }
                                                 @endphp
                                                <div class="col-sm-12">
                                                    ...
                                                    <select class="" id="add-subject" name="subject[]" data-style="btn btn-secondary" multiple="multiple" title="Choose Subjects" data-size="10">
                                                       ...
                                                       @foreach($subjects as $subject))
                                                        <option value="{{$subject->sub_Id}}" {{ (in_array($subject->sub_Id, $subs)) ? 'selected' : ''}}>{{$subject->subject}} </option>
                                                        @endforeach

                                                        {{--@foreach($subs as $sublist){{$sublist == $subject->sub_Id ? 'selected': ''}}   @endforeach--}}
                                                    </select>
                                                </div>
                                            </div>
                                        </div>
